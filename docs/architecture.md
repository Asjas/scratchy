...
websites on hosted/dedicated servers. It is **not** a serverless framework — it
is built for long-running Node.js processes with persistent connections, worker
pools, and in-memory caching.
...
- In-memory caching (LRU, async-cache-dedupe) is effective with long-lived
  processes
...
**Trade-off:** No auto-scaling per-request; scaling is done via horizontal
server instances behind a load balancer.

### 2. Worker Threads for Rendering

**Decision:** Use Piscina Worker Thread pools for SSR and SSG instead of
...
- Worker Threads have their own V8 isolate — garbage collection doesn't affect
  the main thread
...
**Trade-off:** Slightly higher memory usage (each worker has its own V8 heap)
and communication overhead for small payloads.

### 3. Qwik as Primary Renderer (Not React)

**Decision:** Use Qwik for rendering with React as an escape hatch via
`qwikify$()`.
...
**Trade-off:** Smaller ecosystem than React; developers need to learn Qwik's
`$()` convention.

### 4. tRPC for Internal, REST for External

**Decision:** Use tRPC for all internal API communication and Fastify REST
...
- External consumers (third-party integrations) need standard REST with OpenAPI
  docs
...
**Trade-off:** Less "magic" than Prisma — requires writing more explicit
queries.

### 6. Communication Patterns

**Decision:** Support both SharedArrayBuffer+Atomics and Redis for worker
communication.
...
- SharedArrayBuffer is ideal for large payloads (e.g., serialized component
  trees)
- Redis is ideal for cached data that multiple servers need to access

### 7. Convention-Based CLI Scaffolding

**Decision:** Provide CLI commands to scaffold models, views, APIs, and
controllers.
